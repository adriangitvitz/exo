...
parser.add_argument(
    "command", nargs="?", choices=["run", "eval", "train"], help="Command to run"
)
...
parser.add_argument(
    "--save-every", type=int, default=5, help="Save the model every N iterations."
)
parser.add_argument(
    "--data",
    type=str,
    default="exo/train/data/lora",
    help="Directory where training data lives",
)
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Path to a custom checkpoint to load",
)
parser.add_argument(
    "--save-checkpoint-dir",
    type=str,
    default="checkpoints",
    help="Path to a folder where checkpoints are stored",
)
parser.add_argument("--node-id", type=str, default=None, help="Node ID")
parser.add_argument("--node-host", type=str, default="0.0.0.0", help="Node host")
parser.add_argument("--node-port", type=int, default=None, help="Node port")
parser.add_argument(
    "--models-seed-dir", type=str, default=None, help="Model seed directory"
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Path to discovery config json file",
)
parser.add_argument(
    "--wait-for-peers",
    type=int,
...
parser.add_argument(
    "--chatgpt-api-port", type=int, default=52415, help="ChatGPT API port"
)
parser.add_argument(
...
parser.add_argument(
    "--default-temp", type=float, help="Default token sampling temperature", default=0.0
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Comma separated list of allowed node IDs (only for UDP and Tailscale discovery)",
)
parser.add_argument(
    "--interface-type-filter",
    type=str,
    default=None,
    help="Comma separated list of allowed interface types (only for UDP discovery)",
)
parser.add_argument(
    "--system-prompt", type=str, default=None, help="System prompt for the ChatGPT API"
)
...
shard_downloader: ShardDownloader = (
    new_shard_downloader(args.max_parallel_downloads)
...
web_chat_urls = [
    f"http://{ip}:{args.chatgpt_api_port}"
    for ip, _ in get_all_ip_addresses_and_interfaces()
]
if DEBUG >= 0:
...
allowed_interface_types = (
    args.interface_type_filter.split(",") if args.interface_type_filter else None
)
...
if args.discovery_module == "udp":
    ...
    if not args.discovery_config_path:
        raise ValueError(
            f"--discovery-config-path is required when using manual discovery. Please provide a path to a config json file."
        )
    discovery = ManualDiscovery(
        args.discovery_config_path,
        args.node_id,
        create_peer_handle=lambda peer_id,
        address,
        description,
        device_capabilities: GRPCPeerHandle(
            peer_id, address, description, device_capabilities
        ),
    )
topology_viz = (
...
api = ChatGPTAPI(
    ...
    on_chat_completion_request=lambda req_id, __, prompt: topology_viz.update_prompt(
        req_id, prompt
    )
    if topology_viz
    else None,
...
def update_topology_viz(req_id, tokens, __):
    ...
    topology_viz.update_prompt_output(
        req_id, node.inference_engine.tokenizer.decode(buffered_token_output[req_id])
    )
...
def update_prompt_viz(request_id, opaque_status: str):
    ...
    try:
        ...
        if (
            ...
            or status.get("status") != "start_process_prompt"
        ):
            return
        topology_viz.update_prompt(
            request_id,
            status.get("prompt", "corrupted prompt (this should never happen)"),
        )
...
def throttled_broadcast(shard: Shard, event: RepoProgressEvent):
    ...
    if (
        last_event
        and last_event[0] == event.status
        and current_time - last_event[0] < 0.2
    ):
        return
...
async def run_model_cli(node: Node, model_name: str, prompt: str):
    ...
    if not shard:
        print(
            f"Error: Unsupported model '{model_name}' for inference engine {inference_class}"
        )
        return
...
async def run_iter(node: Node, shard: Shard, train: bool, data, batch_size=1):
    ...
    for batch in tqdm(iterate_batches(data, batch_size), total=len(data) // batch_size):
        _, _, lengths = batch
        losses.append(
            np.sum(lengths * await node.enqueue_example(shard, *batch, train=train))
        )
...
async def eval_model_cli(
    node: Node, model_name, dataloader, batch_size, num_batches=-1
):
    inference_class = node.inference_engine.__class__.__name__
    shard = build_base_shard(model_name, inference_class)
    if not shard:
...
async def train_model_cli(
    ...
    train, val, test = dataloader(tokenizer.encode)
    print(
        f"Training on {len(train)} examples with batch_size {batch_size} for {iters} epochs"
    )
...
async def check_exo_home():
    home, has_read, has_write = (
        await ensure_exo_home(),
        await has_exo_home_read_access(),
        await has_exo_home_write_access(),
    )
    if DEBUG >= 1:
        print(f"exo home directory: {home}")
...
async def main():
    ...
    if args.command == "run" or args.run_model:
        ...
        if not model_name:
            print(
                "Error: Model name is required when using 'run' command or --run-model"
            )
            return
        ...
        if args.command == "eval":
            if not model_name:
                print(
                    "Error: Much like a human, I can't evaluate anything without a model"
                )
                return
