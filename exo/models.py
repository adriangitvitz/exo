...
model_cards = {
    ...
    "deepseek-r1-distill-qwen-14b-3bit": {
        "layers": 48,
        "repo": {
    ...
    "deepseek-r1-distill-llama-70b-6bit": {
        "layers": 80,
        "repo": {
...
def get_repo(model_id: str, inference_engine_classname: str) -> Optional[str]:
    return (
        model_cards.get(model_id, {})
        .get("repo", {})
        .get(inference_engine_classname, None)
    )
...
def get_supported_models(
    supported_inference_engine_lists: Optional[List[List[str]]] = None,
) -> List[str]:
    ...
    supported_inference_engine_lists = [
        [
            inference_engine_classes[engine]
            if engine in inference_engine_classes
            else engine
    ...
    def supports_all_engine_lists(model_info: dict) -> bool:
        return all(
            has_any_engine(model_info, engine_list)
            for engine_list in supported_inference_engine_lists
        )
