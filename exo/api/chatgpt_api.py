...
def build_prompt(
    tokenizer, _messages: List[Message], tools: Optional[List[Dict]] = None
):
    ...
    try:
        ...
        prompt = tokenizer.apply_chat_template(**chat_template_args)
        if DEBUG >= 3:
            print(f"!!! Prompt (UTF-8 encoded): {prompt}")
        return prompt
...
class ChatGPTAPI:
    def __init__(
        ...
        self.token_callback.on_next(
            lambda _request_id, tokens, is_finished: asyncio.create_task(
                self.handle_tokens(_request_id, tokens, is_finished)
            )
        )
        ...
        cors.add(
            self.app.router.add_post(
                "/v1/image/generations", self.handle_post_image_generations
            ),
            {"*": cors_options},
        )
        cors.add(
            self.app.router.add_get(
                "/v1/download/progress", self.handle_get_download_progress
            ),
            {"*": cors_options},
        )
        cors.add(
            self.app.router.add_get("/modelpool", self.handle_model_support),
            {"*": cors_options},
        )
        cors.add(
            self.app.router.add_get("/healthcheck", self.handle_healthcheck),
            {"*": cors_options},
        )
        cors.add(
            self.app.router.add_post("/quit", self.handle_quit), {"*": cors_options}
        )
        cors.add(
            self.app.router.add_delete(
                "/models/{model_name}", self.handle_delete_model
            ),
            {"*": cors_options},
        )
        cors.add(
            self.app.router.add_get("/initial_models", self.handle_get_initial_models),
            {"*": cors_options},
        )
        cors.add(
            self.app.router.add_post("/create_animation", self.handle_create_animation),
            {"*": cors_options},
        )
        cors.add(
            self.app.router.add_post("/download", self.handle_post_download),
            {"*": cors_options},
        )
        cors.add(
            self.app.router.add_get("/v1/topology", self.handle_get_topology),
            {"*": cors_options},
        )
        cors.add(
            self.app.router.add_get("/topology", self.handle_get_topology),
            {"*": cors_options},
        )
        ...
        async def middleware(request):
            try:
                return await asyncio.wait_for(
                    handler(request), timeout=self.response_timeout
                )
    ...
    async def handle_model_support(self, request):
        try:
            ...
            await response.prepare(request)
            async for path, s in self.node.shard_downloader.get_shard_download_status(
                ...
                model_data = {
                    s.shard.model_id: {
                        ...
                        if s.downloaded_bytes == s.total_bytes
                        else 100 * float(s.downloaded_bytes) / float(s.total_bytes),
    ...
    async def handle_post_chat_token_encode(self, request):
        ...
        tokenizer = await resolve_tokenizer(
            get_repo(shard.model_id, self.inference_engine_classname)
        )
    ...
    async def handle_get_download_progress(self, request):
        ...
        for node_id, progress_event in self.node.node_download_progress.items():
            if isinstance(progress_event, RepoProgressEvent):
                ...
                print(
                    f"Unknown progress event type: {type(progress_event)}. {progress_event}"
                )
    ...
    async def handle_post_chat_completions(self, request):
        ...
        if DEBUG >= 2:
            print(
                f"[ChatGPTAPI] Handling chat completions request from {request.remote}: {data}"
            )
        ...
        if not shard:
            supported_models = [
                model
                for model, info in model_cards.items()
                if self.inference_engine_classname in info.get("repo", {})
            ]
            return web.json_response(
                {
        ...
        if DEBUG >= 4:
            print(f"[ChatGPTAPI] Resolved tokenizer: {tokenizer}")

        # Add system prompt if set
        if self.system_prompt and not any(
            msg.role == "system" for msg in chat_request.messages
        ):
        ...
        if DEBUG >= 2:
            print(f"[ChatGPTAPI] Processing prompt: {request_id=} {shard=} {prompt=}")

        try:
            await asyncio.wait_for(
                asyncio.shield(
                    asyncio.create_task(
                        self.node.process_prompt(shard, prompt, request_id=request_id)
                    )
                ),
            ...
            if DEBUG >= 2:
                print(
                    f"[ChatGPTAPI] Waiting for response to finish. timeout={self.response_timeout}s"
                )
            ...
            if stream:
                ...
                try:
                    ...
                    while True:
                        if DEBUG >= 2:
                            print(
                                f"[ChatGPTAPI] Waiting for token from queue: {request_id=}"
                            )
                        ...
                        if DEBUG >= 2:
                            print(
                                f"[ChatGPTAPI] Got token from queue: {request_id=} {tokens=} {is_finished=}"
                            )
                        ...
                        if not eos_token_id and hasattr(tokenizer, "_tokenizer"):
                            eos_token_id = tokenizer.special_tokens_map.get(
                                "eos_token_id"
                            )
                        ...
                        if is_finished:
                            finish_reason = (
                                "stop" if tokens[-1] == eos_token_id else "length"
                            )
                        if DEBUG >= 2:
                    ...
                    if DEBUG >= 2:
                        ...
                        traceback.print_exc()
                    return web.json_response(
                        {"detail": f"Error processing prompt: {str(e)}"}, status=500
                    )
                    ...
                    if request_id in self.token_queues:
                        if DEBUG >= 2:
                            print(
                                f"[ChatGPTAPI] Cleaning up token queue: {request_id=}"
                            )
            ...
            async def stream_image(_request_id: str, result, is_finished: bool):
                if isinstance(result, list):
                    ...
                    try:
                        ...
                        try:
                            image_url = request.app.router["static_images"].url_for(
    ...
    async def handle_delete_model(self, request):
        ...
        try:
            if await delete_model(model_id, self.inference_engine_classname):
                ...
                return web.json_response(
                    {"detail": f"Model {model_id} files not found"}, status=404
                )
        except Exception as e:
            if DEBUG >= 2:
                traceback.print_exc()
            return web.json_response(
                {"detail": f"Error deleting model: {str(e)}"}, status=500
            )
    ...
    async def handle_create_animation(self, request):
        try:
            ...
            if not replacement_image_path:
                return web.json_response(
                    {"error": "replacement_image_path is required"}, status=400
                )
            ...
            create_animation_mp4(
                replacement_image_path, output_path, device_name, prompt_text
            )
    ...
    async def handle_post_download(self, request):
        try:
            ...
            if not model_name:
                return web.json_response(
                    {"error": "model parameter is required"}, status=400
                )
    ...
    async def handle_get_topology(self, request):
        try:
            ...
            if DEBUG >= 2:
                traceback.print_exc()
            return web.json_response(
                {"detail": f"Error getting topology: {str(e)}"}, status=500
            )
